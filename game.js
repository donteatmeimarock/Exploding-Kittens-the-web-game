...
const UI = {
    ...
    modalAltBtn: document.getElementById('modal-alt-btn'),
...
function promptNope(action) {
    ...
    showModal("Waiting for Nope...", statusText, "Proceed (No Nopes)", resolveActionStack, html);
    
    setTimeout(() => {
        const btnP1 = document.getElementById('btn-nope-p1');
        if (btnP1 && !btnP1.disabled) btnP1.onclick = () => playNopeFromModal(0);
        
        const btnP2 = document.getElementById('btn-nope-p2');
        if (btnP2 && !btnP2.disabled) btnP2.onclick = () => playNopeFromModal(1);
    }, 0);
...
function render() {
    ...
    UI.opponentHand.innerHTML = '';
    UI.opponentHandCount.textContent = opponent.hand.length;
    ...
    // Update Draw Button
    if (state.actionStack.length > 0 && activePlayer.hand.includes(CardTypes.NOPE)) {
...
function showModal(title, desc, btnText, btnAction, customHtml = '') {
    ...
    UI.modalAltBtn.classList.add('hidden'); // Reset alt btn
